...
def invite_new(request):
    ...
    if request.method == "POST":
        ...
        try:
            Invitation.objects.create(code=code, created_by=request.user)
        except (OperationalError, ProgrammingError):
...
def invite_accept(request):
    ...
    if request.method == "POST":
        if form.is_valid():
            ...
            try:
                Invitation.consume_code_static(code, request.user)
                ...
                # Auto-create conversation with the teacher who created the invite
                try:
                    invitation = Invitation.objects.get(code=code)
                    if invitation.created_by.profile.role == 'teacher':
                    ...
                    pass # Fail silently if chat creation fails, main goal is verification

                messages.success(request, "تم تفعيل حسابك كطالب. يمكنك الآن رفع التسليمات وبدء المحادثة مع المعلم.")
